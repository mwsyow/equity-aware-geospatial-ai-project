...
def get_TAI_scaled():
    """
    Returns a DataFrame with mean travel time scaled for each district across all models.
    """
    ...
    for model_name, path in MODEL_PATHS.items():
        ...
        temp = df[["district_code", "mean_travel_time_mins_scaled"]].rename(
            columns={"mean_travel_time_mins_scaled": model_name}
        )
        if combined_df.empty:
            combined_df = temp
        else:
            combined_df = pd.merge(combined_df, temp, on="district_code", how="outer")
